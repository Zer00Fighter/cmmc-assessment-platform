...
def test_workbook_uses_omni_branding(
    ...
    assert workbook["Cover"]["A1"].value == (
        "Omni by R!SC"
    )
    assert "CMMC Level 2" in (
        workbook["Cover"]["A3"].value
    )

    settings = workbook["Settings"]
    generated_by = {
        settings.cell(row=row, column=2).value:
        settings.cell(row=row, column=3).value
...
def test_default_output_uses_omni_name() -> None:
    ...
    assert builder.output_path.name == (
